/**
 * All the minions at first goes about collecting the first coin
 * it sees. When no visible coins are found, it is ensured that
 * all the minions are the same position. One minion then
 * uses fire, killing every teammate
 */

import java.util.ArrayList;
import java.util.Scanner;

public class Self_killer_with_one_alive {
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            for(int i = 0 ; i < alive_cnt ; i++) {
                if(i > 0) str.append(" | ");
                if(coinx.size() > 0) {
                    str.append(String.format("MOVE %d %d %d", ids.get(i), coinx.get(0), coiny.get(0)));
                }
                else if(i == 0) {
                    str.append(String.format("FIRE %d", ids.get(i)));
                }
                else {
                    str.append(String.format("WAIT %d", ids.get(i)));
                }
            }
